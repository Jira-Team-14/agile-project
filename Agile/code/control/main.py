...
class main_control:
    ...
    # call MODEL to query deleting info and authenticate delete
    def deleteEMployeeControl(self, id):
        queryobj=modifyTable(self.connection)
        result = queryobj.deleteEmployee(id)
        return result

    # call MODEL to query deleting info and authenticate delete
    def deleteBuildingControl(self, id):
        queryobj=modifyTable(self.connection)
        result = queryobj.deletebuilding(id)
        return result


    #function to set the eligiblility of the reserved user to no so that he/she cannot reserve another bed
    def setEligibleNO(self,infotuple):
        queryobj = reservationTable(self.connection)
        queryobj.makeEligibleNo(int(infotuple[2]))
        return

    #function to decrease the available beds in a room
    ...
    #function to add the reservation to the db
